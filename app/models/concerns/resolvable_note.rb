module ResolvableNote
  ...
  included do
    ...
    scope :resolvable, -> { where(type: ['DiffNote', 'ThreadNote']).user.where(noteable_type: 'MergeRequest') }
  ...
  def discussion
    ...
    self.noteable.find_resolvable_discussion(self.discussion_id)
  end
end
