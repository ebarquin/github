module SeedHelper
  ...
  def ensure_seeds
    if File.exists?(GITLAB_GIT_SUPPORT_PATH)
      FileUtils.rm_r(GITLAB_GIT_SUPPORT_PATH)
    end

    FileUtils.mkdir_p(GITLAB_GIT_SUPPORT_PATH)
  ...
  def create_git_attributes
    dir = File.join(GITLAB_GIT_SUPPORT_PATH, 'with-git-attributes.git', 'info')
...
bla/bla.txt
  ...
  def create_invalid_git_attributes
    dir = File.join(GITLAB_GIT_SUPPORT_PATH, 'with-invalid-git-attributes.git', 'info')
