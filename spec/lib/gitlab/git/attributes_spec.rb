...
describe Gitlab::Git::Attributes do
  let(:path) do
    File.expand_path(File.join(GITLAB_GIT_SUPPORT_PATH, 'with-git-attributes.git'))
  end
  ...
  describe '#each_line' do
    ...
    it 'does not yield when the attributes file has an unsupported encoding' do
      path = File.expand_path(File.join(GITLAB_GIT_SUPPORT_PATH, 'with-invalid-git-attributes.git'))
